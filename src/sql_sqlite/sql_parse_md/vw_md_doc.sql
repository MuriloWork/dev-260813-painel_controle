DROP VIEW IF EXISTS vw_md_doc;
CREATE VIEW IF NOT EXISTS vw_md_doc AS
WITH extracted AS (
    SELECT
        b.file_path,
    ...
    FROM tb_json_md_ast_blocks b,
         json_each(b.json_data) AS j
    WHERE json_extract(value, '$.type') NOT IN ('code', 'table', 'thematicBreak')
),
hierarquia AS (
    SELECT
        *,
