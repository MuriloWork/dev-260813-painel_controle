Defines the functional requirements for the project, including different levels of business and end-user requirements and the functional areas of business processes.
- funções de negócio 

# 1. Pipeline do Usuário / Fluxo Operacional

## 1.1. Conceitos MBSE Envolvidos
...
## 1.2. Referências Normativas e Padrões
...
## 1.3. Pipeline do Usuário / Fluxo Operacional
### 1.3.1. Precisamos de um Mapa Mental das Funcionalidades antes dos Pipelines?
...
### 1.3.2. Existe Modelo Normalizado?
...
### 1.3.3. O Mapa Mental Numerado Funciona?
...
### 1.3.4. Exemplo de Estrutura Aninhada para o `crm-cdd`:
...
### 1.3.5. Inclusão da Referência da Funcionalidade no Pipeline
...
### 2.4.3. automatic functions

#### 2.4.3.1. painel controle, section parse
Exemplo:
- parse md [PRD, SRD, session_code_blocks] ast [tables, code blocks] ⟶ [json, sqlite]
- parse code_scripts [ps1, python, dart] ast ⟶ [json, sqlite]  
- parse code_scripts [ps1, python, dart] raw ⟶ [json, sqlite] 

#### 2.4.3.2. painel controle, section session 

##### 2.4.3.2.1. create PRD 

[template_PRD, rules] ⟶ PRD 

##### 2.4.3.2.2. create SRD 
aplicar hierarquia funcional com base nos modelos de dados
[template_SRD, rules] ⟶ SRD 

##### 2.4.3.2.3. lint tools
...
#### 2.4.3.3. painel controle, section sync
Transformações entre os modelos de dados.
- sync json ⟷ sqlite ⟷ csv ⟷ xlsx

### 2.4.4. agent functions
- inteligentes
...
#### 2.4.4.1. project documents rules  
...
#### 2.5.1.2. demais dados, armazenamento fisico
...
#### 2.5.1.3. demais dados, armazenamento temporario no run time 
{conteudo opcional, conforme o escopo do projeto}

#### 2.5.1.4. dados de negocio, modelos para code scripts
...
### 2.5.2. dados de controle 

#### 2.5.2.1. dados de controle, armazenamento fisico
{conteudo obrigatorio, conforme o escopo do projeto}
tipos de dados: [auth, config, actions, state]

##### 2.5.2.1.1. fonte de dados de controle `data_source_path`
**Data source name template**: para o caso de documentos com name template
...
#### 2.5.2.2. dados de controle, armazenamento temporario no run time 
{conteudo opcional, conforme o escopo do projeto}

#### 2.5.2.3. dados de controle, modelos para code scripts

##### 2.5.2.3.1. schemas de extração/importação

##### 2.5.2.3.2. schemas internos para transformações intermediarias

##### 2.5.2.3.3. schemas de exportação
...
#### 3.1.2.2. sobre a seção "contexto do projeto"
informações gerais e referencias do projeto
...
#### 3.1.2.3. sobre a seção "arquitetura do projeto"
**Conceitos base**:
...
#### 3.1.2.4. sobre a seção "arquitetura de funções"
...
#### 3.1.2.5. sobre a seção "arquitetura de dados"
...
#### 3.1.2.6. sobre a seção "descrições funcionais"
...
### 3.2.1. objetivos e resumo 
{from PRD}

### 3.2.2. stack do projeto 
{from PRD}

### 3.2.3. caminhos do projeto (relativos a project_path) 
{from PRD}

## 3.3. arquitetura do projeto 
### 3.3.1. camada frontend
#### 3.3.1.1. src/view_shell/
### 3.3.2. camada backend
#### 3.3.2.1. src/controllers/
#### 3.3.2.2. src/services/
##### 3.3.2.2.1. src/services/~~painel_settings~~painel_init.py
##### 3.3.2.2.2. src/services/parse
##### 3.3.2.2.3. src/services/logs
##### 3.3.2.2.4. src/services/dart_tools
#### 3.3.2.3. src/utils_io/
#### 3.3.2.4. src/sql_sqlite/
##### 3.3.2.4.1. src/sql_sqlite/sql_parse_md
##### 3.3.2.4.2. src/sql_sqlite/sql_tkinter
##### 3.3.2.4.3. src/sql_sqlite/sql_parse_script
#### 3.3.2.5. src/sql_supabase/
#### 3.3.2.6. src/models/
##### 3.3.2.6.1. src/models/schemas
#### 3.3.2.7. src/~~config~~ repositories/
##### 3.3.2.7.1. src/config/schemas
##### 3.3.2.7.2. src/~~utils_auth~~/auth_data*.json
#### 3.3.2.8. src/temp
### 3.3.3. camada dados
#### 3.3.3.1. dbMu/
- files
...
## 3.4. arquitetura de funções

### 3.4.1. resumo
{from PRD}

### 3.4.2. pipeline [function_type]
{from PRD}

### 3.4.3. function type automatic

##### 3.4.3.1.1. function group <function_group_name>
| funcao          | arquivos principais                                    | descricao                                  |
| --------------- | ------------------------------------------------------ | ------------------------------------------ |
| {fucntion_name} | {lista dos code scripts mais relevantes}               | {{responsabilidades principais da função}} |
| {fucntion_name} | {lista das instruções [agent, skills] mais relevantes} | {{responsabilidades principais da função}} |

##### 3.4.3.1.2. classe `class_name`
...
### 3.4.4. function type agent
#### 3.4.4.1. agent `agent_name` 
**Responsabilidade**: responsabilidade do agente 
**Fora do escopo**: descrição breve das responsabilidades que estão fora do escopo
**Prompt**: prompt do agente

##### 3.4.4.1.1. skill `skill_name` 
...
### 3.5.1. dados de negocio 

#### 3.5.1.1. dados fonte, armazenamento fisico
{conteudo obrigatorio, conforme o escopo do projeto}

##### 3.5.1.1.1. fonte de dados de negocio `data_source_path`
**Data source name template**: para o caso de documentos com name template
...
#### 3.5.1.2. dados transformados, armazenamento fisico

##### 3.5.1.2.1. documentos de dados
{conteudo opcional, conforme o escopo do projeto}
geralmente documentos json

##### 3.5.1.2.2. banco de dados `database_name`
{conteudo opcional, conforme o escopo do projeto}

###### 3.5.1.2.2.1. tabela `table_name`
{conteudo opcional, conforme o escopo do projeto}
...
#### 3.5.1.3. dados transformados, armazenamento temporario no run time 
{conteudo opcional, conforme o escopo do projeto}

#### 3.5.1.4. modelos para code scripts

##### 3.5.1.4.1. schemas de extração/importação
**Responsabilidade**: Documentar os schemas declarativos usados pelo sistema (ast_model, json_schema_*).
...
##### 3.5.1.4.2. schemas internos para transformações intermediarias

##### 3.5.1.4.3. schemas de exportação

### 3.5.2. dados de controle [auth, config, actions, state]


## 3.6. descrições funcionais

### 3.6.1. lógicas, responsabilidades e requisitos por classe/metodo
